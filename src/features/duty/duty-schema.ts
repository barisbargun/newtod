...
export function useDutyCreateSchema() {
  ...
  return z.object({
    name: z.string().min(3, validationMsg('min', 3)).max(150, validationMsg('max', 150)),
    color: z
      .string()
      .regex(/^#([0-9A-F]{3}|[0-9A-F]{6})$/i, 'Must be a valid 3-digit or 6-digit hex color'),
    icon: z.string().min(3, validationMsg('min', 3)).max(150, validationMsg('max', 150))
  })
}
